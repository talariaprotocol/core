...
describe('EarlyAccessCodes', function () {
  ...
  it.only('Debe consumir un código correctamente', async function () {
    ...
    expect(await number.number()).to.be.equal(0)

    // Execute the test function!
    await expect(
      number.connect(owner).setNumber(transfer.commitment, proof, root, nullifierHash, ['0x'], 1),
    ).to.emit(number, 'Success')

    expect(await number.number()).to.be.equal(1)
  })
})
